...
$(document).ready(function(){
	...
	//检查试卷状态－－－如果是设计状态，则提醒先发布，再收集
	var surveyState=$("#surveyState").val();
	if(surveyState==="0"){
		$("body").append("<div id=\"myDialogRoot\"><div class='dialogMessage'>您试卷当前是设计状态，需要先发布才能收集答卷。<br/>是否确认发布试卷！</div></div>");
		var myDialog=$( "#myDialogRoot" ).dialog({
			...
			buttons: {
				...
				"CENCEL":{
                    ...
                    click: function() {
                      //$( this ).dialog( "close" );
                    	window.location.href=ctx+"//mySurveys";
                    }
				}
			},
	...
	$(".sur_edit").click(function(){
		...
		$("body").append("<div id=\"myDialogRoot\"><div class='dialogMessage'>您试卷已经发布，确认要重新编辑使试卷回到设计状态。<br/>是否确认返回设计状态！</div></div>");
		var myDialog=$( "#myDialogRoot" ).dialog({
			width:500,
			height:230,
			autoOpen: true,
